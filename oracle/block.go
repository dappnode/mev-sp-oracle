...
// claims are settled as accumulated minus claimed, so a recomputed total below
// what an address already claimed would brick its future claims.
//
// Apply the detection only from the slot where it was first deployed. Before it,
// forced payments are handled by the exception table above, exactly as the
