...
* **contract**: Gets all the rewards produced by the members of the pool and provides a set of functions to subscribe, unsubscribe and claim rewards by providing the appropriate proofs of belonging to the merkle tree.

## Configuration parameters

The oracle shall contain the following configuration parameters, that will determine how rewards are calculated:
...
* `PayableDonation`: A donation was made to the smoothing pool contract, sent as a normal transaction detected in the payable.
* `NonPayableDonation`: A donation was made to the smoothing pool contract, sent via a smart contract, which is not seen in the payable.
...
* `AutoSubscription`: On the other hand, a validator can start earning rewards from the very beginning if it adds `COLLATERAL_GWEI` amount as collateral. This collateral can be deposited by calling the register function in the oracle smart contract from any account, but specifying the correct amount and a valid `validatorIndex`. This transaction will be detected by the oracle, and will subscribe the validator, starting to accrue `PendingRewards` just one block after being recognized. This type of subscription allows the validator to start earning rewards without having to wait weeks or months until a proposal is detected. Note that the subscription is only considered valid if `collateral>=COLLATERAL_GWEI`, which its added to the validator `PendingRewards`. This collateral is returned after the first valid block proposal. See `PendingRewards`, `AccumulatedRewards`. Note that any address can subscribe a validator to the pool, but just calling this smart contract function.


**Unsunscribing** from the pool:
* `Unsubscribe`: Similarly, the oracle shall detect the following event from the smoothing pool smart contract, which signals that a given `validatorId` was unsubscribed from the pool. Note that only `sender` = TODO: deposit address? should successfully unsubscribe a validator.
...
When a validator has an active subscription to the pool (`Active` or `YellowCard` state) it is eligible for rewards, meaning that it will receive a given share of each reward that is sent to the pool.
...
* `AccumulatedRewards`: This rewards are already consolidated, meaning that they can be claimed at any time, by submitting a valid proof to the smart contract.
...
TODO: Write some notes on the smart contract.
https://github.com/dappnode/mev-sp-contracts

### Test vectors

* TODO: test vector for merkle trees, proofs, hashes, roots.
* TODO: test vector for rewards calculation + collaterals + fees
...
> Q: If I dont propose blocks, do I earn?

A: You will accumulate `PendingRewards` over the time, but you won't be able to claim the until you have succesfully proposed a block. If the block you propose contains an invalid fee recipient, then your `PendingRewards` will be reset and shared equally among all pool participants.
